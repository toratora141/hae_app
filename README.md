...
6. 判定結果には、その判定に使われた写真の縮小サムネイルが一緒に表示されるので、どの1枚に対する
   結果かが分かる。👍/👎でフィードバックを記録でき、「ログをJSONで書き出す」でこれまでの記録
   (このサムネイルを含む)を保存できる
...
「映え度(hae_score)」だけでなく、他にどんな項目でAPIの判定精度が良いか試したいという要望から、
`docs/questions.json` に以下の4項目を追加しました。**これらはhae_photoで確認された質問ではなく、
今回新しく追加したものです**。実際のAPIが期待どおりの構造で答えてくれるかは未確認です。

| キー | type | 内容 |
|---|---|---|
| `skill_level` | choice | プロ/アマチュア/スナップ写真のどれに見えるか |
| `lighting_quality` | score | 光の使い方(明暗のコントロール)の良さ |
| `color_harmony` | score | 配色・色味の統一感 |
| `background_clutter` | score | 背景のすっきり度(高いほどすっきり) |

判定結果画面にもそれぞれの値を表示し、ログ(`docs/app.js` の `appendLog`)にも生の値と
(choice型の場合)確率を記録しています。SNS映え確率(`sns_worthy`)のように極端な値しか
返ってこない項目があれば、これらの追加項目とログを見比べて「どのプロパティなら実用的な精度・分布に
なるか」を判断する材料にしてください。

なお「シーン(scene)によって尋ねる項目を変えてはどうか」というアイデアも検討しましたが、
今回は全項目を毎回まとめて送る形のままにしています。理由は、シーンごとに質問を出し分けるロジックを
先に作り込むより、まず全項目・全シーンでログを取って「シーンごとに精度がどう違うか」を確認してから
出し分けを設計する方が手戻りが少ないためです。`appendLog` は `scene` と各追加項目を同じレコードに
記録しているので、書き出したログをシーンでフィルタすれば精度の違いは今の形でも確認できます。
...
以前のバージョンでは、ログには数値(scene・hae_score等)だけを記録しており、どの写真に対する結果
なのかを後から見分ける手段がありませんでした。これを解決するため、判定リクエストのたびに送信画像を
長辺 `160px` / JPEG quality `0.5` 程度まで縮小したサムネイル(data URL文字列)を1枚作り、
判定結果画面にその場で表示するとともに、ログの各レコード(`thumbnail` フィールド)にも一緒に保存する
ようにしました。「ログをJSONで書き出す」でエクスポートしたJSONにもこのサムネイルがそのまま含まれるため、
書き出したファイルだけで「どの写真に対してどの数値が出たか」を後から確認できます。

- サムネイルはAPIには送らず(送信用の画像は従来どおり別途 `maxSide` 設定に従って生成)、
  この端末のブラウザの `localStorage` にのみ保存されます。リポジトリやサーバーには送られません
- `localStorage` の容量上限(端末により数MB程度)に達すると保存に失敗するため、`saveLog` は保存に
  失敗した場合、古いレコードから間引いて再試行するようにしました(容量超過時にログ全体が保存できず
  消えてしまうことを避けるため)。上限に達した場合は古い記録(サムネイル含む)から順に失われます
...
`tests/guide.test.js` で `parseAnswers`(実フィクスチャ使用・追加判定項目のケース含む)・`cellToRC`・
`selectTemplate`・`buildTemplateMatchQuestion`・`resolveTemplate`(手動選択/サーバー判定/自動選択の優先順位)・
`buildInstruction`(左右上下・サイズ・不明・達成の各ケース)を検証しています。29件すべてパスすることを
確認済みです。
...
- 「自動送信/今すぐ判定」の操作パネル(`#controls`)は、カメラ映像・センサー許可ボタンのすぐ下に
  配置した(判定操作に素早くアクセスできるように、指標・ガイド・結果より上に表示する構成とした)
