...
5. 画面が静止すると自動で1枚だけAPIに送信され、判定結果と画角ガイド(矢印・枠)、主指示(1つに絞った
   案内文)が表示される
...
動作確認だけしたい場合は、URLに `?mock=1` を付けて開くとAPIを呼ばず疑似応答で一通り動作します
...
「確率0.4未満のscene/subject_posは不明として扱う」という仕様における「確率」は、
...
判定がちらつきやすい問題がありました。これを緩和するため、`docs/guide.js` に以下を追加しました。
...
「今いちばん優先して直すべきこと」を、1つに絞って表示するようにしました。ロジックは
...
「伝わった」「分かりにくい」の小さなボタンを1組置いており、押すと表示中の指示 IDに紐づけて記録されます
(任意、複数回押しても構いません)。

Playwrightのフェイクカメラを使い、`?mock=1` で質問セットA/B/C×表示モードsingle/multiの全造6通りを
...
非標準の拡張API)を使い、対応している端末・ブラウザだけで動作します。
...
これまでのログ(`haeApp.log.v1`、数値だけの1判定=1レコード形式)に代えて、`haeApp.log.v2` として
...
撮影の3秒後まで「この写真に満足?👍/👎」という帯を表示し、押すとその撮影(`shutterId`)に紐づけて
...
7. 帰宅後(または移動中の空き時間に)、「ログをJSONで書き出す」でファイルを保存する
...
1. `?mock=1` で質問セットA/B/C×表示モードsingle/multiの全造6通りが、コンソールエラー無く
...
ただし、これは合成カメラ映像(フェイクデバイス)・モックまたは擬似APIサーバーでの確認であり、
...
- `instruction_resolved` の `achieved` 判定は、device系は仕様の達成しきい値(ブレ50以上・傾き2°未満・
  明るさ30以上)、`server:main_problem:*`はその値が次の判定で選ばれなくなったとき、`server:noul:*`は
...
- `scripts/summarize_log.mjs` で `instruction_resolved`/`instruction_feedback`/`judge` を質問セット×
  ...
  切り替えない前提の近似であり、厄密な値ではないことをスクリプトの出力・本READMEの両方に明記した)
...
- Cloudflare Workers中継の実デプロイ・実際の疏通(手順は書いたが未デプロイ)
...
- 新設した `skill_level`/`lighting_quality`/`color_harmony`/`background_clutter` の各質問に対して、
  APIが期待どおりの構造・妥当な分布(SNS映え確率のように極端な値に偿らないか)で答えてくれるかどうか
